...
class phpblm { private $blm; private $header = array(); private $def = array(); private $data = array(); public function __construct($file) { $this->blm = file_get_contents($file);
		$this->splitPieces();
	}
	...
	private function splitPieces() {
		...
		for ($i=0; $i<$datac; $i++) { $row = explode($this->header['EOF'], trim($data[$i]));
			for ($j=0; $j<count($row); $j++) { $row_arr[$this->def[$j]] = $row[$j];
			}
			$this->data[] = $row_arr;
		}
	}
